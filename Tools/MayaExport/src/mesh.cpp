	...
	MStatus Mesh::writeOgreBinary(ParamList &params)
	{
		...
		if (params.useSharedGeom)
		{
			for(i=0; i<m_sharedGeom.vertices.size(); i++)
			{
				...
				bbox.merge(Ogre::Vector3(v->x,v->y,v->z));
			}
		}
		else
		{
			for(i=0; i<m_submeshes.size(); i++)
			{
				for(j=0; j<m_submeshes[i]->m_vertices.size(); j++)
				{
					vertex* v = &m_submeshes[i]->m_vertices[j];
					bbox.merge(Ogre::Vector3(v->x,v->y,v->z));
				}
			}
		}
...
/*	// Write mesh data to Ogre XML
	MStatus Mesh::writeXML(ParamList &params)
	{
		...
		if (params.useSharedGeom)
		{
			...
			params.outMesh << "\t\t<vertexbuffer positions=\"true\">\n";
			...
			if (params.exportVertNorm || params.exportTexCoord || params.exportVertCol)
			{
				params.outMesh << "\t\t<vertexbuffer normals=";
				if (params.exportVertNorm)
					params.outMesh << "\"true\"";
				else 
		...
		for (i=0; i<m_submeshes.size(); i++)
		{
			if (m_submeshes[i]->name() != "")
				params.outMesh << "\t\t<submeshname name=\"" << m_submeshes[i]->name().asChar() 
					<< "\" index=\"" << i << "\"/>\n";
		}
		params.outMesh << "\t</submeshnames>\n";
		// write blend shapes
		if (params.exportBlendShapes)
		{
			params.outMesh << "\t<poses>\n";
			if (params.useSharedGeom)
			{
				// for all blend shapes applied on shared geometry, write all poses
				for (i=0; i<m_sharedGeom.dagMap.size(); i++)
				{
					BlendShape* pBS = m_sharedGeom.dagMap[i].pBlendShape;
					if (pBS)
					{
						for (j=0; j<pBS->getPoses().size(); j++)
						{
							...
							params.outMesh << "\t\t<pose target=\"mesh\" index=\"-1\"";
							if (p.name != "")
								params.outMesh << " name=\"" << p.name.asChar();
							params.outMesh<< ">\n";
							// write vertex offsets
							for (k=0; k<p.offsets.size(); k++)
							{
								vertexOffset vo = p.offsets[k];
								params.outMesh << "\t\t\t<poseoffset index=\"" << vo.index << "\" x=\"" << vo.x << "\" y=\"" 
									<< vo.y << "\" z=\"" << vo.z << "\"/>\n";
							}
							params.outMesh << "\t\t</pose>\n";
						}
			...
			{
				// for all submeshes, write all poses used by its blend shape deformer
				for (i=0; i<m_submeshes.size(); i++)
				{
					BlendShape* pBS = m_submeshes[i]->m_pBlendShape;
					if (pBS)
					{
						for (j=0; j<pBS->getPoses().size(); j++)
						{
							pose& p = pBS->getPoses()[j];
							// write pose attributes
							params.outMesh << "\t\t<pose target=\"submesh\" index=\"" << p.index << "\"";
							if (p.name != "")
								params.outMesh << " name=\"" << p.name.asChar();
							params.outMesh<< ">\n";
							// write vertex offsets
							for (k=0; k<p.offsets.size(); k++)
							{
								...
								if (params.exportTexCoord)
								{
									indexMultiplier += m_submeshes[i]->m_vertices[0].texcoords.size();
								}
								params.outMesh << "\t\t\t<poseoffset index=\"" << vo.index*indexMultiplier << "\" x=\"" << vo.x << "\" y=\"" 
									<< vo.y << "\" z=\"" << vo.z << "\"/>\n";
							}
							params.outMesh << "\t\t</pose>\n";
						}
