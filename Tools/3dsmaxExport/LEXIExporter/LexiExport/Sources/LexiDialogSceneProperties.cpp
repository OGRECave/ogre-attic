...
CScenePropertiesDlg::CScenePropertiesDlg(Window* pParent) : Dialog(IDD_DIALOG_SCENEPROPERTIES, DlgProc, pParent)
{
...
CScenePropertiesDlg::~CScenePropertiesDlg()
{
...
void CScenePropertiesDlg::Init(CDDObject *pMeta, const char *pszDefExt)
{
	// Hookup data
	m_sDefExt=pszDefExt;
...
void CScenePropertiesDlg::SetInstance(CDDObject *pData, CExportObject* pObj)
{
	m_pData=pData;
	m_pObj=pObj;
...
//
INT_PTR CALLBACK CScenePropertiesDlg::DlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	CScenePropertiesDlg* pThis = (CScenePropertiesDlg*)Window::GetMapping(hWnd);

	switch(message)
	{
	case WM_INITDIALOG:
		{
			pThis = (CScenePropertiesDlg*)lParam;
...
//
void CScenePropertiesDlg::OnInitDialog()
{
	GetClientRect(m_OrgClientRect);
}

void CScenePropertiesDlg::OnSize()
{
...
//
void CScenePropertiesDlg::OnChanged(const CDDObject *pInstance, const char *pszKey)
{
	::SendMessage(m_hParent, WM_NOTIFY_MESSAGE_ID, 0, 0);
}

//
void CScenePropertiesDlg::BrowseNode()
{	
...
void CScenePropertiesDlg::BrowseOutput()
{	
	// We need to have a valid instance
	if(!m_pObj || !m_pData) return;
	OPENFILENAME ofn;
