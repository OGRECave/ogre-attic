...
# Blender: 241
# Group: 'Export'
# Tooltip: 'Export meshes and animations to Ogre3D'
# """
...
try:
	...
	try:
		...
		class ArmatureAnimationProxyManager(AnimationProxyManager):
			...
			def toAnimations(self, animationExporter):
				for key in self.animationProxyKeyList:
					self.animationProxyDict[key].toAnimation().export()
				return
			def getActions(self):
			...
			def export(self, exportPath, materialScriptName, colouredAmbient, gameEngineMaterials, convertXML, copyTextures):
				...
				if len(self.selectedList):
					...
					for name in self.selectedList:
						...
						#TODO: export armature and armature animations
						if self.armatureAnimationProxyManagerDict.has_key(name):
							Log.getSingleton().logWarning("Armature animations are not yet supported!")
