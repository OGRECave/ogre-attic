...
namespace Ogre{
    ...
    {
    public:
        /// used to make sure we shut after main()
        struct Shutdown
        {
            MemProfileManager* mPtr;
            ~Shutdown()
            {
                mPtr->~MemProfileManager();
            }
        };
    ...
    public:
        ...
        {
            if(!smInstance)
                smInstance  = ::new(malloc(sizeof(MemProfileManager))) MemProfileManager();
            return *smInstance;
        }

        /// @return singleton instance pointer
        static MemProfileManager* getSingletonPtr()
        {
            assert(smInstance);
            return smInstance;
        }
