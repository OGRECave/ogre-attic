...
namespace Ogre {
	...
	/** Pre-transforms and batches up meshes for efficient use as instanced geometry
		in a scene
	@remarks
		Shader instancing allows to save both memmory and draw call. While static geometry stores
		500 times the same object in a batch to display 500objects, this shader Instancing implementation
		stores only 80 times the object, and then re-use the vertex data with different shader parameter.
		Although you save memmory, you make more draw call. However, you still make less draw call than
		if you were rendering each object independantly.
	@par
	...
	@par
		The bounding boxes informations are computed whith object position only. 
		It doesn't take care of the object orientation. 
	@par
	...
	*/
	...
	public:
		...
		*/
		...
		class _OgreExport  InstanciedObject
		{
			 ...
			 enum TransformSpace
        {
			...
			/// list of Geometry Buckets that contains the instancied object
		...
		public:
			InstanciedObject(int index);
			InstanciedObject(int index,SkeletonInstance *skeleton,AnimationStateSet*animations);
			~InstanciedObject();
		...
		*/
		class _OgreExport  BatchInstance : public MovableObject
		{
			...
			enum ShaderType
			{
				/// the material has only a vertex shader
				ST_VERTEX_U,
				/// the material has lighted vertex shader
				ST_VERTEX_L
			};

			/// list of LOD Buckets in this BatchInstance
			typedef std::vector<LODBucket*> LODBucketList;
			typedef std::map<int, InstanciedObject*> ObjectsMap;
		protected:
		...
		protected:
			...
			/// List of lights for this BatchInstance
...
//			const Vector3& getCentre(void) const { return mCentre; }
		...
		//	uint32 getTypeFlags(void) const;
			...
			void attachToScene();
			void addInstanciedObject(int index, InstanciedObject* object);
			InstanciedObject*  isInstanciedObjectPresent(int index);
			InstanciedObject** getObjectsAsArray(unsigned short & size);
	...
	public:
        ...
        */
        ...
        virtual uint8 getRenderQueueGroup(void) const;
			/** make any material an Instanced Material 
		@remarks
		This method create a new material from a material that doesn't support Crowd.
		@note
		no note
		*/
		String getInstancedMaterialName(const String &originalMaterialName);
