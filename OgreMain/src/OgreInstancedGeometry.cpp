...
namespace Ogre {
	...
	void InstancedGeometry::build(void)
	{
		...
		// Now make each material of each LodBucket of each BatchInstance instanced.
		for (QueuedSubMeshList::iterator qi = mQueuedSubMeshes.begin();
			qi != mQueuedSubMeshes.end(); ++qi)
		{
			QueuedSubMesh* qsm = *qi;
			
			qsm->materialName = getInstancedMaterialName (qsm->materialName);
		}
	...
	//--------------------------------------------------------------------------
	String InstancedGeometry::getInstancedMaterialName(const String &originalMaterialName)
	{
			...
			GpuProgramParametersSharedPtr params = p->getVertexProgramParameters ();
 			params->clearAutoConstants();
				if(mBaseSkeleton.isNull())
				{
				    ...
				    params->setNamedAutoConstant ("worldMatrix3x4Array[0]", GpuProgramParameters::ACT_WORLD_MATRIX_ARRAY_3x4);
				}
				else
				{
	...
	void InstancedGeometry::addBatchInstance(void)
	{
		...
		{
			InstanciedObject* instancedObject = ret->isInstanciedObjectPresent(objIt->first);
			if(instancedObject == NULL)
			{
				if(mBaseSkeleton.isNull())
				{
					instancedObject=new InstanciedObject(objIt->first);
				}
				else
				{
					instancedObject=new InstanciedObject(objIt->first,mSkeletonInstance,mAnimationState);
				}
				ret->addInstanciedObject(objIt->first,instancedObject);
			}
		...
		while (lodIterator.hasMoreElements())
		{
			...
			while (matIt.hasMoreElements())
			{
				...
				while(geomIt.hasMoreElements())
				{
					...
					//now setups the news InstanciedObjects.
					for(objIt=ret->getInstancesMap().begin();objIt!=ret->getInstancesMap().end();objIt++)
					{
						//get the destination IntanciedObject
						InstanciedObject*obj=objIt->second;
						InstanciedObject::GeometryBucketList::iterator findIt;
	...
	//--------------------------------------------------------------------------
	void InstancedGeometry::BatchInstance::setShaderType(InstancedGeometry::BatchInstance::ShaderType type)
	{
			BatchInstance::LODIterator lodIterator = getLODIterator();
				//parse all the lod buckets of the BatchInstance
				while (lodIterator.hasMoreElements())
				{
					...
					while (matIt.hasMoreElements())
					{
						...
						while(geomIt.hasMoreElements())
						{
							...
							switch( type )
							{
								case ShaderType::ST_VERTEX_U:
									suffix="/VERTEX_U";
									break;
								case ShaderType::ST_VERTEX_L:
									suffix="/VERTEX_L";
								break;
							}
							...
							while (pIt.hasMoreElements())
							{
								...
								case ShaderType::ST_VERTEX_U:
										...
										params = p->getVertexProgramParameters ();
 										params->clearAutoConstants();
										params->setNamedAutoConstant ("ViewProjMatrix", GpuProgramParameters::ACT_VIEWPROJ_MATRIX);
								        params->setNamedAutoConstant ("worldMatrix3x4Array[0]", GpuProgramParameters::ACT_WORLD_MATRIX_ARRAY_3x4);
										p->setShadowCasterVertexProgram("InstancingShadowCaster");
	...
	//--------------------------------------------------------------------------
	InstancedGeometry::InstanciedObject::InstanciedObject(int index,SkeletonInstance *skeleton, AnimationStateSet*animations):mIndex(index),
	...
	//--------------------------------------------------------------------------
	InstancedGeometry::InstanciedObject::InstanciedObject(int index):mIndex(index),
		mTransformation(Matrix4::ZERO),
		mPosition(Vector3::ZERO),
		mScale(Vector3(1,1,1)),
		mOrientation(Quaternion::IDENTITY),
	...
	//--------------------------------------------------------------------------
	InstancedGeometry::InstanciedObject::~InstanciedObject()
	{
	...
	//--------------------------------------------------------------------------
	void InstancedGeometry::InstanciedObject::addBucketToList(GeometryBucket*bucket)
	{
		mGeometryBucketList.push_back(bucket);
	}
	//--------------------------------------------------------------------------
	void InstancedGeometry::InstanciedObject::setPosition( Vector3  position)
	{
	...
	//--------------------------------------------------------------------------
	void InstancedGeometry::InstanciedObject::translate(const Vector3 &d)
	{
		mPosition += d;
		needUpdate();
	}
	//--------------------------------------------------------------------------
	void InstancedGeometry::InstanciedObject::translate(const Matrix3 & axes,const Vector3 &move)
	{
		Vector3 derived = axes * move;
        translate(derived);
	}
	//--------------------------------------------------------------------------
	Matrix3 InstancedGeometry::InstanciedObject::getLocalAxes() const
	{
	...
	//--------------------------------------------------------------------------
	Vector3 & InstancedGeometry::InstanciedObject::getPosition(void)
	{
		return mPosition;
	}
	//--------------------------------------------------------------------------
	void InstancedGeometry::InstanciedObject::yaw(const Radian&angle)
	{
		Quaternion q;
	...
	//--------------------------------------------------------------------------
	void InstancedGeometry::InstanciedObject::pitch(const Radian&angle)
	{
		Quaternion q;
	...
	//--------------------------------------------------------------------------
	void InstancedGeometry::InstanciedObject::roll(const Radian&angle)
	{
		Quaternion q;
	...
	//--------------------------------------------------------------------------
	void InstancedGeometry::InstanciedObject::setScale(const Vector3&scale)
	{
		mScale=scale;
		needUpdate();
	}
	//--------------------------------------------------------------------------
	void InstancedGeometry::InstanciedObject::rotate(const Quaternion& q)
	{
	...
	//--------------------------------------------------------------------------
	void InstancedGeometry::InstanciedObject::needUpdate()
	{
	...
	//--------------------------------------------------------------------------
	void InstancedGeometry::InstanciedObject::updateAnimation(void)
	{
	...
	//--------------------------------------------------------------------------
	AnimationState* InstancedGeometry::InstanciedObject::getAnimationState(const String& name) const
    {
        if (!mAnimationState)
        {
            OGRE_EXCEPT(Exception::ERR_ITEM_NOT_FOUND, "Object is not animated",
                "InstancedGeometry::InstanciedObject::getAnimationState");
        }
	...
	//--------------------------------------------------------------------------
	void InstancedGeometry::BatchInstance::addInstanciedObject(int index,InstanciedObject* object)
	{
		mInstancesMap[index]=object;
	}
	//--------------------------------------------------------------------------
	 InstancedGeometry::InstanciedObject* InstancedGeometry::BatchInstance::isInstanciedObjectPresent(int index)
	{
	...
	//--------------------------------------------------------------------------
	InstancedGeometry::InstanciedObject** InstancedGeometry::BatchInstance::getObjectsAsArray(unsigned short & size)
	{
		size = static_cast <unsigned short> (mInstancesMap.size());
		ObjectsMap::iterator it;
		InstanciedObject** array=new InstanciedObject*[size];
		int i=0;
	...
	{
		ObjectsMap::iterator it;
		//we parse the Instancied Object map to update the animations.
		 ...
		 mIndexData(0)
	{
		...
		for(size_t i=0;i<mRenderOp.vertexData->vertexDeclaration->getElementCount();i++)
		{
	...
	unsigned short InstancedGeometry::GeometryBucket::getNumWorldTransforms(void) const 
	{
		if(mBatch->getBaseSkeleton().isNull())
		{
			...
			return batch->getInstancesMap().size();
		}
		else
		{
			BatchInstance* batch=mParent->getParent()->getParent();
			return mBatch->getBaseSkeleton()->getNumBones()*batch->getInstancesMap().size();
		}
	}
	//--------------------------------------------------------------------------
	...
	void InstancedGeometry::GeometryBucket::build()
	{
		...
		for (gi = mQueuedGeometry.begin(); gi != giend; ++gi)
		{
			...
			InstanciedObject* instancedObject = mParent->getParent()->getParent()->isInstanciedObjectPresent(index);
			if(instancedObject == NULL)
			{
				if(mBatch->getBaseSkeleton().isNull())
				{
					instancedObject=new InstanciedObject(index);
				}
				else
				{
					instancedObject=new InstanciedObject(index,mBatch->getBaseSkeletonInstance(),
						mBatch->getBaseAnimationState());
				}
				mParent->getParent()->getParent()->addInstanciedObject(index,instancedObject);
