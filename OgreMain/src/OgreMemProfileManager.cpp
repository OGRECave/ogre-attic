...
// realise statics
Ogre::MemProfileManager* Ogre::MemProfileManager::smInstance = NULL;

// make sure we shutdown at exit
static Ogre::MemProfileManager::Shutdown sManager;
...
namespace Ogre{
    ...
    void MemProfileManager::update()
    {
		...
		tmpStats.profileID = 0;
		tmpStats.numAllocations =0;
		tmpStats.numBytesAllocated =0;
		tmpStats.numDeallocations =0;
		tmpStats.numBytesDeallocated =0;

		uint32 f00 = mProfArray.size();
    ...
    void MemProfileManager::shutdown()
    {
		...
		mReportLog.close(); // close the log
    }
