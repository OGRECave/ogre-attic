...
Copyright (c) 2006 The OGRE Team
...
namespace Ogre
{
	...
	static Matrix4 computeConstrainedProjection( const Vector4& pinhole, 
												 ...
												 const std::vector<Vector2>& constraint)
	{
		...
		if(!solveNxNLinearSysDestr(11, mat, col)) 
		{
		...
		if(incrPrecision)
		{
			for (int k=0; k<3; k++)
			{
				...
				if(!solveNxNLinearSysDestr(11, mat, nvec)) 
				{
		...
		if(!solveNxNLinearSysDestr(4, mat, zrow)) 
		{
...
// Class implementations ___________________________________________________
