...
void TerrainSceneManager::setWorldGeometry( const String& filename )
{
  ...
  {
    printf( "Error: Invalid heightmap size : %d,%d... should be 2^n+1, 2^n+1\n",
  ...
  {
    printf( "Error: Image is not a grayscale image.\n" );
    return ;
  }
  ...
  if ( world_texture != "" )
    mTerrainMaterial -> addTextureLayer( world_texture );

  if ( detail_texture != "" )
    mTerrainMaterial -> addTextureLayer( detail_texture );
